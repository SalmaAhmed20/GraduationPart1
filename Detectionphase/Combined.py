...
def Violence_Part1(Frames_data, Predicated_data, Processing_times):
    ...
    with mp_pose.Pose(
        ...
        while video.isOpened():
            ...
            try:
                ...
                frame_rate = 24
            frame = cv2.cvtColor(ori_image, cv2.COLOR_BGR2RGB)
            frame = cv2.resize(frame, (128, 128)).astype("float32")
            frame = frame.reshape(128, 128, 3) / 255
            ...
            image = cv2.putText(image, text, (35, 50), FONT, 1.25, text_color, 3)
